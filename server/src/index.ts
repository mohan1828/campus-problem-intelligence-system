...
// Auth Middleware (simulated session via headers for the prototype)
app.use((req, res, next) => {
  const role = req.headers['x-user-role'];
  const userId = req.headers['x-user-id'];
  if (role && userId) {
    (req as any).user = { id: userId, role: role };
  }
  next();
});
...
app.post('/api/login', async (req, res) => {
  ...
  // Try finding by email (for admin/staff) or by registerNumber (for student) or name (fallback)
...
app.get('/api/users', async (req, res) => {
  const users = await prisma.user.findMany({ select: { id: true, name: true, email: true, role: true, department: true, registerNumber: true, year: true } });
  res.json(users);
});
...
app.get('/api/complaints', async (req, res) => {
  // If student, only return their own complaints, unless Admin/Staff
